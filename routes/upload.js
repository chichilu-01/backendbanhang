...
// [POST] /media/upload - Upload media mới (admin-only)
...
router.patch("/:id/set-main", verifyToken, isAdmin, async (req, res) => {
  ...
  try {
    ...
    await query("UPDATE product_media SET is_main = true WHERE id = ?", [mediaId]);
...
// [GET] /media/product/:id - Lấy danh sách media
