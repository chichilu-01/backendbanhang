...
router.get("/suggest", async (req, res) => {
  ...
  try {
    const rows = await query("SELECT name FROM products WHERE name LIKE ? LIMIT 10", [`%${keyword}%`]);
...
router.post("/filters/save", verifyToken, async (req, res) => {
  ...
  if (!name || !filter) return res.status(400).json({ error: "Thiếu tên hoặc dữ liệu bộ lọc" });
...
router.post("/", verifyToken, async (req, res) => {
  ...
  try {
    const result = await query(
      `INSERT INTO products (name, price, description, image_url, images, sizes, colors, stock)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        name,
        price,
...
router.put("/:id", verifyToken, async (req, res) => {
  ...
  try {
    await query(
      `UPDATE products SET name=?, price=?, description=?, image_url=?, images=?, sizes=?, colors=?, stock=? WHERE id=?`,
      [
        name,
        price,
        description,
